...
app.controller('CompanyController', function ($scope, $http) {
  ...
  $scope.updateCompany = function () {
    $http.put(`${API_URL}/${$scope.editingCompany.id}`, $scope.editingCompany).then(response => {
      const index = $scope.companies.findIndex(c => c.id === response.data.id);
  ...
  $scope.deleteCompany = function (id) {
    $http.delete(`${API_URL}/${id}`).then(() => {
      $scope.companies = $scope.companies.filter(c => c.id !== id);
    });
  };
